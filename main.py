...
import time
import numpy as np
from resultat import afficher_resultat
...
def commencer(message):
    ...
    #pour que le joueru puisse finir aussi si l'ia à été trop rapide
        # --- FIN DE PARTIE ---
    # Permet au joueur de finir après l'IA
    running = True
    while running:
        running = player_ctrl.handle_events()
        player_win = player_ctrl.render()
    ...
    # État final
    joueur_a_resolu = not player_ctrl.game_model.verif_win()
    ia_a_resolu = not ai_ctrl.ia_model.verif_win()

    if joueur_a_resolu and not ia_a_resolu:
        afficher_resultat("joueur") 
    elif ia_a_resolu and not joueur_a_resolu:
        afficher_resultat("ai")     
    else:
        afficher_resultat("")
